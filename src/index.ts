...
import { registerEditCommand } from './commands/edit'; // Import edit command
import { db } from './lib/db';
import { version } from '../package.json';
...
async function main() {
    ...
    registerEditCommand(program); // Register edit command
    ...
    // Use `program.processedArgs` which commander sets after parsing succeeds
    // This check handles cases where arguments are passed but don't match any known command/option
     const userArgs = process.argv.slice(2);
     if (userArgs.length > 0 && !program.processedArgs && program.args.length === 0) {
         // Check if it was a help/version flag that commander handles internally
         if (!['-h', '--help', '-V', '--version'].includes(userArgs[0])) {
             console.error('Invalid command: %s\nSee --help for a list of available commands.', userArgs[0]);
             process.exit(1);
         }
