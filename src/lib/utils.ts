...
export function parseTimeAndCombineWithDate(timeStr: string, referenceDate: Date): Date | null {
    if (isNaN(referenceDate.getTime())) {
         console.error(chalk.red("Invalid reference date provided for parsing time."));
         return null;
    }
    ...
    if (!match) {
        console.error(chalk.red(`Invalid time format: "${timeStr}". Use HH:MM AM/PM or HH:MM.`));
        return null;
    }
    ...
    if (isNaN(hour) || isNaN(minute) || minute < 0 || minute > 59) {
         console.error(chalk.red(`Invalid hour or minute in time: "${timeStr}".`));
        return null;
    }
    ...
    if (ampm) {
        ...
        if (hour < 1 || hour > 12) {
            console.error(chalk.red(`Invalid 12-hour format hour: ${hour} in "${timeStr}".`));
            return null;
        }
        if (ampm === 'PM' && hour !== 12) {
            hour += 12;
        ...
        if (hour < 0 || hour > 23) {
             console.error(chalk.red(`Invalid 24-hour format hour: ${hour} in "${timeStr}".`));
             return null;
        }
    }
...
export function formatDuration(totalSeconds: number): string {
    const secondsInt = Math.floor(Number(totalSeconds));
    if (secondsInt < 0 || isNaN(secondsInt)) return chalk.gray('N/A');
    if (secondsInt === 0) return '0s';
    const hours = Math.floor(secondsInt / 3600);
    const minutes = Math.floor((secondsInt % 3600) / 60);
    const seconds = Math.floor(secondsInt % 60);
    let parts: string[] = [];
...
export function parseFilterString(filterStr: string): ParsedFilter | null {
    const match = filterStr.trim().match(/^(\w+)\s*([=><!]=?)\s*(.+)$/);
    if (!match) {
        console.error(chalk.red(`Invalid filter format: "${filterStr}".`));
        return null;
    }
    ...
    if (!supportedOperators.includes(operator)) {
         console.error(chalk.red(`Unsupported operator "${operator}" in filter: "${filterStr}".`));
         return null;
    }
     if ((key === 'date' || key === 'status') && operator !== '=') {
        console.error(chalk.red(`Operator "${operator}" is not supported for key "${key}". Only '=' is allowed.`));
    ...
    const parsed: ParsedFilter = { key, operator, value };
    ...
    if (key === 'date' && operator === '=') {
        ...
        if (lowerValue === 'today') targetDate = today;
        else if (lowerValue === 'yesterday') {
    ...
    if (['duration', 'total', 'avg'].includes(key)) {
        ...
        parsed.valueSeconds = parseDurationToSeconds(value);
        if (parsed.valueSeconds === null) {
            console.error(chalk.red(`Invalid duration format "${value}" for key "${key}".`));
            return null;
        }
    }
...
export function parseDurationToSeconds(durationStr: string): number | null {
    const trimmedStr = durationStr.trim().toLowerCase();
    const hhmmMatch = trimmedStr.match(/^(\d{1,3}):(\d{2})$/);
    if (hhmmMatch) {
        const hours = parseInt(hhmmMatch[1], 10);
        const minutes = parseInt(hhmmMatch[2], 10);
        if (!isNaN(hours) && !isNaN(minutes) && minutes >= 0 && minutes < 60 && hours >=0) {
